      ...
      <div class="main-content">
        <section class="section">
          <div class="section-header">
            <h1>Edit Private Sub Category </h1>
            <div class="section-header-breadcrumb">
              <div class="breadcrumb-item active"><a href="{{ route('admin.private_category.index') }}">Private Category</a></div>
              <div class="breadcrumb-item">Edit Private Sub Category</div>
            </div>
          </div>

          <div class="section-body">
            <a href="{{ route('admin.private_sub_category.index') }}" class="btn btn-primary"><i class="fas fa-list"></i> Private Sub Category</a>
            <div class="row mt-4">
                <div class="col-12">
                  <div class="card">
                    <div class="card-body">
                        <form action="{{ route('admin.private_sub_category.update', $subCategory->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="form-group col-12">
                                    ...
                                    <select name="category" id="category" class="form-control">
                                        ...
                                        @foreach ($categories as $category)
                                            <option {{ $subCategory->private_category_id  == $category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group col-12">
